...
使用已有 NOCIX 账户时，任务会按两阶段完成登录：先提交一次账户和密码，
再提交一次账户和密码。第二阶段完成后，如果任务状态显示
`waiting_for_email_code`，请从该账户的邮箱中获取邮件验证码，并在对应的任务卡中输入。

- 邮件验证码只在当前运行期间保存在内存中，不会写入 SQLite、`.env`、浏览器存储、日志或 Telegram 通知。
- 输入正确的验证码后，任务会在同一个浏览器会话中继续执行；输入错误时可以重新输入并重试。
- 验证超时、取消验证或 VPS 重启都会清除当前登录会话；此时必须重新启动任务，不能从原会话继续。
- 仅输入验证码不会提交真实订单。订单仍需经过后续库存检查和明确的下单流程。
...
每次发布或合并用户可见变更时，请将同一条安全摘要同时写入根目录的
`CHANGELOG.md` 和 `frontend/src/lib/updates.ts`，并放在各自列表的最前面。
两处记录应使用相同的日期、提交短 hash、更新类型以及中英文标题和说明。
只记录用户可见的功能、修复、文档或安全变化，不要写入 API Key、账户密码、
代理凭据、一次性验证码、Telegram 凭据或其他敏感值。前端更新页使用本地数据，
不会在运行时请求 GitHub。
